...
def test_mastodon_found(monkeypatch):
    # Patch the resolver to simulate a found account
    fake = {"status":"found","url":"https://mastodon.social/@alice"}
    with patch("maigret_sites_example.mastodon_resolver.resolve_mastodon", return_value=fake):
        res = check("alice")
        assert res["http_status"] == 200
        assert res["ids_usernames"] == {"alice": "username"}
        assert res["parsing_enabled"] is True

def test_mastodon_not_found(monkeypatch):
